<template>
    <div :class="[locale == 'fa' ? 'md:items-end' : 'md:items-start']" class="mt-4 flex flex-wrap items-center justify-center gap-y-8 sm:gap-2 md:my-2 md:flex md:flex-col articleSection3">
        <div v-for="article in articles" :key="article.id" :class="['group',{'animateArticleBox3':showCategoryItems}]">
...
const data : any = await $fetch(`${apiBase}/landing/mostFavorite`)
...
const setupObserver = () => {
    ...
    const category = document.querySelector('.articleSection3') 
    if(category) {
