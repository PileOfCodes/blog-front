<template>
    <div :class="[locale == 'fa' ? 'md:items-end' : 'md:items-start']" class="mt-4 flex flex-wrap items-center justify-center gap-y-8 sm:gap-2 md:my-2 md:flex md:flex-col articleSection">
        <div v-for="(article, index) in articles" :key="article.id" :class="['group',{'animateArticleBox':showCategoryItems}]">
            <NuxtLink :to="`/posts/${article.slug}`" class="relative" :style="[locale == 'fa' ? 'font-family: irsMedium' : 'font-family: sans-serif']">
                <div :style="[locale == 'fa' ? 'direction:rtl' : 'direction:ltr']" class="text-slate-800 dark:text-white  font-bold transition-all duration-300">
                    <div :class="[locale == 'fa' ? 'absolute bottom-[10%] sm:bottom-[5%] right-2 ' : 'absolute bottom-[10%] sm:bottom-[5%] left-2 ']" class="opacity-0 text-sm lg:text-xs w-[240px] lg:w-[140px] md:text-xs md:w-[100px] truncate group-hover:opacity-100 font-bold group-hover:duration-500 transition-all sm:truncate">{{ locale == 'fa' ? article.persianTitle : article.englishTitle }}</div>
...
const data: any = await $fetch(`${apiBase}/landing/newest`)
...
const setupObserver = () => {
    ...
    const category = document.querySelector('.articleSection') as HTMLDivElement | null
    if(category) {
...
.animateArticleBox:nth-of-type(1) {
    animation-name: articleBox;
    opacity: 0;
}
.animateArticleBox:nth-of-type(2) {
    animation-name: articleBox;
    animation-delay: 300ms;
    opacity: 0;
}
.animateArticleBox:nth-of-type(3) {
    animation-name: articleBox;
    animation-delay: 500ms;
    opacity: 0;
}
.animateArticleBox:nth-of-type(4) {
    animation-name: articleBox;
