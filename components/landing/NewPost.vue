        ...
        <div v-for="(article, index) in articles" :key="article.id" :class="['group',{'animateArticleBox':showCategoryItems}]">
            <NuxtLink to="#" class="relative" :style="[locale == 'fa' ? 'font-family: irsMedium' : 'font-family: sans-serif']">
                <div :style="[locale == 'fa' ? 'direction:rtl' : 'direction:ltr']" class="text-slate-800 dark:text-white  font-bold transition-all duration-300">
                    <div :class="[locale == 'fa' ? 'absolute bottom-[10%] sm:bottom-[5%] right-2 ' : 'absolute bottom-[10%] sm:bottom-[5%] left-2 ']" class="opacity-0 text-sm lg:text-xs w-[240px] lg:text-sm lg:w-[240px] md:text-xs md:w-[100px] truncate group-hover:opacity-100 font-bold group-hover:duration-500 transition-all sm:truncate">{{ locale == 'fa' ? article.persianTitle : article.englishTitle}}</div>
